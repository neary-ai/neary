...
class DocumentSearch(BasePlugin):
    def __init__(self, id, conversation_id, services, settings=None, data=None):
        ...
        self.document_manager = DocumentManager(conversation_id=conversation_id)

    @snippet
    def insert_similar_documents(self, context):
        max_results = self.settings["insert_similar_documents"]["max_results"]
        query = context.get_user_message()

        if query:
            document_data = []
            context_docs = self.document_manager.similar_search(query)
    ...
    async def load_url(self, url):
        try:
            await self.document_manager.load_url(url)
