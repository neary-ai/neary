...
class DevTools(BasePlugin):
    ...
    @tool
    async def generate_plugin_files(
        self, plugin_name, display_name, description, author, icon=None, url=None
    ):
        ...
        toml_template = toml_template.replace(
    ...
    async def generate_tool(
        ...
        try:
            ...
            settings_dict = (
                {setting["name"]: setting for setting in settings} if settings else {}
            )
            ...
            function_signature = ",\n".join(
                [
                    f"    {param['name']}: {type_mapping.get(param['type'], param['type'])}"
