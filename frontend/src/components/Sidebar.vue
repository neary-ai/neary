<template>
    <!-- Sidebar -->
    <aside v-show="store.sidebarOpen" class="z-10 hidden fixed sm:flex sm:flex-col h-full pb-4 pt-[4.5rem] bg-nearyblue-300 border-r border-slate-400/10 shadow-lg w-44 items-center justify-between">
        <Spaces />
        <div class="flex gap-3 items-center justify-between text-slate-400/80">
            <Square3Stack3DIcon @click="toggleManageSpaces()" class="hover:text-slate-300/80 cursor-pointer w-5 h-5" />
                ...
                <div class="absolute inset-0">
                    <div class="fixed inset-y-0 left-0 flex max-w-full">
                        <TransitionChild as="template" enter="transform transition ease-in-out duration-500 sm:duration-700"
                            ...
                            <DialogPanel
                                class="w-60 bg-nearyblue-200 flex flex-col items-start justify-between shadow-xl">
                                <div class="w-full">
                                    <div class="w-full flex items-start justify-between p-6 px-4">
                                      <img @click="router.push('/');" :src="logo" class="w-[5rem]" />
                                        <button type="button" class="rounded-md text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-0"
                                ...
                                <div class="flex gap-3 items-center justify-center text-slate-400/80 w-full pb-4">
                                    <Square3Stack3DIcon @click="toggleManageSpaces()" class="cursor-pointer w-5 h-5" />
...
import { Square3Stack3DIcon, UserIcon } from '@heroicons/vue/24/solid';
